...
local trsliterCMD = {['q'] = 'й',['w'] = 'ц',['e'] = 'у',['r'] = 'к',['t'] = 'е',['y'] = 'н',['u'] = 'г',['i'] = 'ш', ['o'] = 'щ',['p'] = 'з',['a'] = 'ф',['s'] = 'ы',['d'] = 'в',['f'] = 'а',['g'] = 'п',['h'] = 'р',['j'] = 'о',['k'] = 'л',['l'] = 'д',['z'] = 'я',['x'] = 'ч',['c'] = 'с',['v'] = 'м',['b'] = 'и',['n'] = 'т',['m'] = 'ь',['/'] = '.'}--,['yo'] = 'ё',['Cz'] = 'Ц',['cz'] = 'ц', ['ia'] = 'я', ['ea'] = 'и',['Ya'] = 'Я', ['ya'] = 'я', ['ove'] = 'ав',['ay'] = 'эй', ['rise'] = 'райз',['oo'] = 'у', ['Oo'] = 'У', ['Ee'] = 'И', ['ee'] = 'и', ['Un'] = 'Ан', ['un'] = 'ан', ['Ci'] = 'Ци', ['ci'] = 'ци', ['yse'] = 'уз', ['cate'] = 'кейт', ['eow'] = 'яу', ['rown'] = 'раун', ['yev'] = 'уев', ['Babe'] = 'Бэйби', ['Jason'] = 'Джейсон', ['liy'] = 'лий', ['ane'] = 'ейн', ['ame'] = 'ейм'}
...
list_cmd = {u8"mh", u8"r", u8"rb", u8"mb", u8"hl", u8"post", u8"mc", u8"narko", u8"recep", u8"osm", u8"dep", u8"sob", u8"tatu", u8"vig", u8"unvig", u8"muteorg", u8"unmuteorg", u8"gr", u8"inv", u8"unv", u8"time", u8"exp", u8"vac", u8"info", u8"za", u8"zd", u8"ant", u8"strah", u8"cur", u8"hall", u8"hilka"}
...
local list_theme = {u8"Фиолетовая", u8"Синяя", u8"Красная", u8"Голубая", u8"Оранжевая", u8"Чёрно-белая"}
--chat
...
--Departament
local dep = {
	list = {"Все Гос. Стуктуры", "Экстренная", "Для связи с МО/МЮ", "Для связи с Т.С.Р", "Собеседование", "[Инфо] - Тех. неполадки","/gov - Новости"},
...
setCmdEdit = {
	[1] = {
		sec = {"2100", "2100", "2100"},
		text = {"Не волнуйтесь, сейчас я Вам помогу!", "/do В правой руке медицинский саквояж.", "/me открыв саквояж, вытащил необходимое средство и передал пациенту напротив"},
	},
}
...
function ButtonMenu(desk, bool) -- подсветка кнопки выбранного меню
	...
	if bool then
		...
		retBool = imgui.Button(desk, imgui.ImVec2(140, 36))
...
function imgui.BeforeDrawFrame()
  if fa_font == nil then
    ...
    fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/lib/fontawesome-webfont.ttf', 17.0, font_config, fa_glyph_ranges)
  end
end
...
--	repeat wait(100) until sampIsLocalPlayerSpawned()
	...
	------------
		...
		if not doesDirectoryExist(dirml.."/MedicalHelper/Отыгровки/") then
			print("{F54A4A}Ошибка. Отсутствует папка. {82E28C}Создание папки для редактированных отыгровок")
			createDirectory(dirml.."/MedicalHelper/Отыгровки/")
		end
...
/medcard #playerID #healID #timeID #money]]  
	...
	colBut							   = ImVec4(0.46, 0.11, 0.29, 1.00)
	colButActive					   = ImVec4(0.59, 0.10, 0.35, 1.00)
	...
	colBut							   = ImVec4(0.26, 0.59, 0.98, 0.40)
	colButActive					   = ImVec4(0.06, 0.53, 0.98, 1.00)
	...
	colBut							   = ImVec4(0.98, 0.26, 0.26, 0.40)
	colButActive					   = ImVec4(0.98, 0.06, 0.06, 1.00)
	...
	colBut							   = ImVec4(0.26, 0.98, 0.85, 0.40)
	colButActive					   = ImVec4(0.06, 0.98, 0.82, 1.00)
	...
	colBut							 = ImVec4(0.51, 0.36, 0.15, 1.00)
	colButActive					 = ImVec4(0.78, 0.55, 0.21, 1.00)
	...
	colBut							   = ImVec4(0.51, 0.51, 0.51, 0.60)
	colButActive					   = ImVec4(0.67, 0.67, 0.67, 1.00)
...
styleWin()
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Скрипт инициализирован!", 0xFF8FA2)
		repeat wait(100) until sampIsLocalPlayerSpawned()
		_, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
		myNick = sampGetPlayerNickname(myid)
		sampAddChatMessage(string.format("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: С возвращением, %s. Для активации главного меню пропишите в чат {22E9E3}/"..cmdBind[1].cmd, sampGetPlayerNickname(myid):gsub("_"," ")), 0xFF8FA2)
		wait(200)
		if buf_nick.v == "" then 
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Обнаружилось, что у Вас не настроена основная информация.", 0xFF8FA2)
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Зайдите в главном меню в раздел \"Настройки\" и заполните необходимую информацию.", 0xFF8FA2)
		end
		--// ПРОВЕРКА ОБНОВЛЕНИЙ
...
function imgui.OnDrawFrame()
	if mainWin.v then
		...
		imgui.SetNextWindowSize(imgui.ImVec2(870, 455), imgui.Cond.FirstUseEver)
		imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.ICON_HEARTBEAT .. " Medical Helper by Kane "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			--///// Func menu button
			imgui.BeginChild("Mine menu", imgui.ImVec2(155, 0), true)
				...
				imgui.Separator()
					imgui.Spacing()
				if ButtonMenu(fa.ICON_HEADPHONES .. u8"  Дискорд", select_menu[7]) then select_menu = {false, false, false, false, false, false, true, false, false} end
					imgui.Spacing()
				imgui.Separator()
					imgui.Spacing()
			...
			imgui.BeginGroup()
				...
				imgui.TextColoredRGB("{FFFFFF}Информация о Вас"); 
				imgui.Separator()
				imgui.Separator()
					imgui.Dummy(imgui.ImVec2(0, 5))
					imgui.Indent(10)
					imgui.Text(fa.ICON_ADDRESS_CARD .. u8"  Имя и Фамилия сотрудника: ");
						imgui.SameLine();
			...
			imgui.BeginChild("settig", imgui.ImVec2(0, 390), true)
				...
				imgui.Dummy(imgui.ImVec2(0, 3))
				if imgui.CollapsingHeader(u8"Цвет темы") then
					mainTheme()
				end
				imgui.Dummy(imgui.ImVec2(0, 3))
			...
			if imgui.Button(u8"Сохранить", imgui.ImVec2(692, 25)) then 
			settingMassiveSave()
			...
			for i,v in ipairs(chgName.rank) do
				...
				f:flush()
				f:close()
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Настройки сохранены.", 0xFF8FA2)
				needSave = false
			end
			imgui.PopStyleColor(1)
			imgui.EndGroup()
			...
			imgui.BeginGroup()
				...
				imgui.EndChild();
					if cmdBind[selected_cmd].rank <= num_rank.v+1 and cmdBind[selected_cmd].rank ~= 1.5 then
						...
						if imgui.Button(u8"Назначить клавишу", imgui.ImVec2(160, 23)) then 
							imgui.OpenPopup(u8"MH | Установка клавиши для активации");
							lockPlayerControl(true)
							editKey = true
						end
						imgui.SameLine();
						if imgui.Button(u8"Очистить активацию", imgui.ImVec2(160, 23)) then 
						...
						if imgui.Button(u8"Изменить команду", imgui.ImVec2(160, 23)) then 
						...
						imgui.Button(u8"Изменить команду", imgui.ImVec2(160, 23))
						imgui.PopStyleColor(3)
						end
						--//Редактировать отыгровку
						imgui.SameLine();
						if cmdBind[selected_cmd].cmd ~= cmdBind[selected_cmd].cmd then
						--if cmdBind[selected_cmd].cmd ~= "r" and cmdBind[selected_cmd].cmd ~= "rb" and cmdBind[selected_cmd].cmd ~= "time" 
						--and cmdBind[selected_cmd].cmd ~= cmdBind[4].cmd and cmdBind[selected_cmd].cmd ~= cmdBind[6].cmd and cmdBind[selected_cmd].cmd ~= cmdBind[11].cmd
						--and cmdBind[selected_cmd].cmd ~= cmdBind[12].cmd and cmdBind[selected_cmd].cmd ~= cmdBind[24].cmd and cmdBind[selected_cmd].cmd ~= cmdBind[1].cmd then
						local numbe = 1
						if imgui.Button(u8"Редактировать отыгровку", imgui.ImVec2(185, 23)) then 
							for i,v in ipairs(setCmdEdit) do
								while setCmdEdit[1].text[numbe] ~= nil do
									chgCmdSet[numbe].v = u8(setCmdEdit[1].text[numbe])
									chgCmd[numbe].v = setCmdEdit[1].sec[numbe] / 1000
									numbe = numbe + 1
								end
							end
						...
						imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImColor(156, 156, 156, 200):GetVec4())
						imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImColor(156, 156, 156, 200):GetVec4())
						imgui.Button(u8"Редактировать отыгровку", imgui.ImVec2(185, 23))
						imgui.PopStyleColor(3)
						end
						imgui.SameLine();
					else 
			...
			if select_menu[6] then
				...
				imgui.BeginChild("help but", imgui.ImVec2(0,0), true)
					...
					imgui.TextWrapped(u8"\tЦеновая политика актуальна для сервера Surprise, если у Вас сервер другой, пожалуйста измените значения.")
					--
					imgui.Bullet(); imgui.SameLine()
			...
			if select_menu[9] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Bullet()
					imgui.TextColoredRGB("Благодарность blast.hk и человеку, который изначально написал этот скрипт!")
					imgui.Bullet()
					...
					--imgui.TextColoredRGB("{FF0000}ВНИМАНИЕ!{FFFFFF} Если Вы установили данный скрипт не с официального форума {32CD32}Arizona RP{FFFFFF} \n      или не с официального канала {32CD32}Arizona RP{FFFFFF} в Discord, то срочно поменяйте пароль и \n      переустановите игру! После чего скачайте скрипт с официального форума {32CD32}Arizona RP{FFFFFF}.")
						...
						if imgui.Button(u8"Удалить скрипт", imgui.ImVec2(166, 26)) then 
							...
							sampAddChatMessage("", 0xFF8FA2)
							sampAddChatMessage("", 0xFF8FA2)
							sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Внимание! Подтвердите удаление командой {77DF63}/mh-delete.", 0xFF8FA2)
							mainWin.v = false
			...
			imgui.BeginChild("discord", imgui.ImVec2(0, 0), false)
				...
				imgui.TextWrapped(u8"Нажмите 'Загрузить для Windows', если хотите использовать программу на компьютере, или 'Открыть в браузере', если хотите пользоваться Дискордом прямо в браузере, но учтите, что в браузере Вы не сможете говорить по активации на клавишу.")
				imgui.Bullet()
			...
			--///Редактор команды
				if imgui.BeginPopupModal(u8"MH | Редактирование команды", null, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoMove) then
					...
					imgui.Text(u8"Введите новую комманду на этот бинд, которую Вы пожелаете."); imgui.Separator()
	...
	local sw, sh = getScreenResolution()
		...
		imgui.Begin(u8"MH | Редактирование отыгровки", actingOutWind, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove);
	...
	imgui.BeginChild("redactor", imgui.ImVec2(1085, 492), true)
		imgui.Columns(3, "acting out", true);
			imgui.SetColumnWidth(-1, 70);
			imgui.Text(u8"№ Отыгр.");
			imgui.NextColumn()
			imgui.SetColumnWidth(-1, 870);
			...
			imgui.Separator();
				...
				--/////1
			local numOut = 1
			while setCmdEdit[1].text[numOut] ~= nil do
				imgui.SameLine()
				imgui.Dummy(imgui.ImVec2(0, 2))
				imgui.Text(u8("["..numOut.."]"));
					if numOut == #setCmdEdit[1].text then
					imgui.SameLine()
						if imgui.Button("-".."##deleteOut", imgui.ImVec2(20, 20)) then
						
						end
					end
				imgui.NextColumn();
				imgui.PushItemWidth(860)
				imgui.InputText("##chat"..numOut, chgCmdSet[numOut])
				imgui.NextColumn(); 
				imgui.PushItemWidth(80)
				imgui.DragFloat("##sleep"..numOut, chgCmd[numOut], 0.1, 0.5, 10.0, u8"%.1f сек.")
				imgui.SameLine()
					if imgui.Button("-".."##"..numOut, imgui.ImVec2(20, 20)) and chgCmd[numOut].v ~= 0.5 then chgCmd[numOut].v = chgCmd[numOut].v - 0.1 end
					imgui.SameLine()
			...
			numOut = numOut + 1
				...
				imgui.Text(u8("["..numOut.."]"));
				imgui.SameLine()
					if imgui.Button("+".."##deleteOut", imgui.ImVec2(20, 20)) then end
				imgui.NextColumn();
				imgui.PushItemWidth(860)
				imgui.Text(u8("/hl [id]"))
				imgui.NextColumn();
		...
		imgui.EndChild()
			...
			--for i,v in ipairs(chgName.sec) do
				setCmdEtid[1].sec = chgCmd
		--	end
			--for i,v in ipairs(chgName.inp) do
				setCmdEtid[1].text = chgCmdSet
			--end
				local f = io.open(dirml.."/MedicalHelper/Отыгровки/hl.med", "w")
			...
			imgui.SameLine();
			if imgui.Button(u8"Сбросить", imgui.ImVec2(358, 25)) then
			
			end
			imgui.SameLine();
			if imgui.Button(u8"Отменить и закрыть", imgui.ImVec2(358, 25)) then actingOutWind.v = not actingOutWind.v end
		--[[
		imgui.TextColored(imgui.ImVec4(1,0.52,0,1), "{myID}")
		imgui.SameLine()
		if imgui.IsItemHovered(0) then setClipboardText("{myID}") end
		imgui.TextColoredRGB("{C1C1C1} - Ваш id - {ACFF36}")
		
		imgui.Spacing()	
		imgui.TextColored(imgui.ImVec4(1,0.52,0,1), "{myNick}")
		imgui.SameLine()
		if imgui.IsItemClicked(0) then setClipboardText("{myNick}");  end
		imgui.TextColoredRGB("{C1C1C1} - Ваш полный ник (по анг.) - {ACFF36}")
		--]]
		imgui.End()
	end
			...
			imgui.BeginGroup()
				...
				if imgui.Button(u8"Начать", imgui.ImVec2(60, 25)) then
					if sobes.selID.v ~= "" then
						...
						sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Укажите id игрока для начала собеседования.", 0xFF8FA2)
					end
				end
	...
	if depWin.v then
		...
		imgui.Begin(fa.ICON_SIGNAL .. u8" Меню рации департамента.", depWin, imgui.WindowFlags.NoResize);
			imgui.BeginGroup()
				imgui.BeginChild("dep list", imgui.ImVec2(225, 225), true)
				...
				imgui.EndChild()
					...
					elseif dep.bool[7] then
						...
						if imgui.Button(u8"Подать", imgui.ImVec2(225, 30)) then
							...
							-- 	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Gov Новости назначить можно только от 0 до 15 минут.", 0xFF8FA2)
							-- 	return
							-- end
...
setCmdEdit[1].sec = chgCmd
setCmdEdit[1].text = chgCmdSet
end
...
function ButtonDep(desk, bool) -- подсветка кнопки выбранного меню
	...
	if bool then
		...
		imgui.PopStyleColor(1)
	elseif not bool then
		 retBool = imgui.Button(desk, imgui.ImVec2(215, 205))
	end
	return retBool
end
...
function sobesRP(id)
	...
	sobes.nextQ = false
		...
		nick = sampGetPlayerNickname(sobes.selID.v) 
			...
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Данного игрока не существует, либо это Вы!", 0xFF8FA2)
			end
			table.insert(sobes.logChat, "{FFC000}Вы: {FFFFFF}Пригласили игрока в организацию.")
			sobes.input.v = ""
			sobes.player = {name = "", let = 0, zak = 0, work = "", bl = "", heal = "", narko = 0.1}
...
function onHotKeyCMD(id, keys)
	if thread:status() == "dead" then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: В данный момент проигрывается отыгровка.", 0xFF8FA2)
	end
end
...
local function playBind(tb)
	if not tb.debug.file or #tb.debug.close > 0 then
		if not tb.debug.file then
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Файл с текстом бинда не обнаружен. ", 0xFF8FA2)
		elseif #tb.debug.close > 0 then
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Диалог, начало которого является строка №"..tb.debug.close[#tb.debug.close]..", не закрыт тегом {dialogEnd}", 0xFF8FA2)
		end
		addOneOffSound(0, 0, 0, 1058)
		return false
	end
...
function binderCmdStart()
	...
	--sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: factCommand: "..factCommand.." fcomRussia: "..factCommandRussia, 0xFF8FA2)
...
funCMD = {} 
function funCMD.del()
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Вы успешно удалили скрипт.", 0xFF8FA2)
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Выгрузка скрипта из игры...", 0xFF8FA2)
	os.remove(scr.path)
	scr:reload()
end
function funCMD.lec(id)
	...
	if not u8:decode(buf_nick.v):find("[а-яА-Я]+%s[а-яА-Я]+") then
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Подождите-ка, сначала нужно заполнить базовую информацию. {90E04E}/mh > Настройки > Основная информация", 0xFF8FA2)
		return
	end
	if id:find("%d+") then
		thread = lua_thread.create(function()
		local secTime = 1
			while setCmdEdit[1].text[secTime] ~= nil do
				sampSendChat(setCmdEdit[1].text[secTime])
				wait(setCmdEdit[1].sec[secTime])
				secTime = secTime + 1
			end
				--sampSendChat("Не волнуйтесь, сейчас я Вам помогу!")
				--wait(2100)
				--sampSendChat("/do В правой руке медицинский саквояж.")
				--wait(2100)
				--sampSendChat("/me открыв саквояж, вытащил"..chsex("","а").." необходимое средство и передал"..chsex("","а").." пациенту напротив")
				--wait(1100)
				sampSendChat("/heal "..id.." "..buf_lec.v)
		end)
	else
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[5].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.strah(id)
	...
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				...
				wait(1000)
					sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения или {FF8FA2}Page Down{FFFFFF}, чтобы закончить диалог.", 0xFF8FA2)
					addOneOffSound(0, 0, 0, 1058)
					local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить")
					while true do
						wait(0)
	...
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[28].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.cur(id)
	...
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
				sampSendChat(string.format("Здравствуйте, меня зовут %s, сейчас я окажу Вам экстренную помощь!", u8:decode(buf_nick.v)))
				wait(2000)
	...
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[29].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.ant(id)
	...
	if id:find("%d+") then
		thread = lua_thread.create(function()
			...
			wait(1000)
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Введите количество выдаваемых антибиотиков.", 0xFF8FA2)
			sampSetChatInputEnabled(true)
			sampSetChatInputText("/antibiotik "..id.." ")
		end)
	else
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[27].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.vac(id)
	...
	if id:find("%d+") then
		thread = lua_thread.create(function()
			if not isCharInModel(PLAYER_PED, 416) then
			local counVacc = 1
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Нажмите {00E600}1{FFFFFF} если ставите первую вакцину. Нажмите {00E600}2{FFFFFF} если вторую.", 0xFF8FA2)
			local len = renderGetFontDrawTextLength(font, "Вакцинация {8ABCFA}Вторая или первая?")
					...
					wait(0)
						...
						if isKeyJustPressed(VK_1) and not sampIsChatInputActive() and not sampIsDialogActive() then counVacc = 1; break end
						if isKeyJustPressed(VK_2) and not sampIsChatInputActive() and not sampIsDialogActive() then counVacc = 2; break end
					end
	...
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[23].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.med(id)
	...
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[7].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.narko(id)
	if thread:status() ~= "dead" then 
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[8].cmd.." [id игрока].", 0xFF8FA2)
		end
end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(500)
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Нажмите на цифру верхней цифровой панели равная количеству выдаваемых рецептов.", 0xFF8FA2)
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[9].cmd.." [id игрока].", 0xFF8FA2)
		end
end
function funCMD.post(stat)
	...
	if not isCharInModel(PLAYER_PED, 416) then -- not
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Чтобы заступить на мобильный пост, Вам необходимо сначала сесть в карету.", 0xFF8FA2)
		addOneOffSound(0, 0, 0, 1058)
	else
		...
		elseif bool then
			if stat:find(".+") then
				...
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Укажите обстановку, например, /"..cmdBind[6].cmd.." Спокойно.", 0xFF8FA2)
			end
		end
	end
end
	...
	if thread:status() ~= "dead" then 
		...
		if id:find("(%d+)") then
			thread = lua_thread.create(function()
				...
				wait(1000)
					addOneOffSound(0, 0, 0, 1058)
					sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Паспорт игрока автоматически закроется, после чего продолжится диалог..", 0xFF8FA2)
					repeat wait(0) until sampIsDialogActive()
					while sampIsDialogActive() do
						local memory = require 'memory'
						memory.setint64(sampGetDialogInfoPtr()+40, bool and 1 or 0, true)
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[13].cmd.." [id игрока].", 0xFF8FA2)
		end	
end
function funCMD.warn(text)
	...
	if num_rank.v+1 < 8 then
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Данная команда Вам недоступна. Поменяйте должность в настройках скрипта, если это требуется.", 0xFF8FA2)
		return
	end
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[14].cmd.." [id игрока] [причина].", 0xFF8FA2)
		end
end
function funCMD.uwarn(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[15].cmd.." [id игрока].", 0xFF8FA2)
		end
end
function funCMD.inv(id)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[19].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.unv(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[20].cmd.." [id игрока] [причина].", 0xFF8FA2)
		end
end
function funCMD.mute(text)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[16].cmd.." [id игрока] [время в минутах] [причина].", 0xFF8FA2)
		end
end
function funCMD.umute(id)
	...
	if num_rank.v+1 < 8 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[17].cmd.." [id игрока].", 0xFF8FA2)
	end
end
function funCMD.rank(text)
	...
	if num_rank.v+1 < 9 then
		...
		sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[18].cmd.." [id игрока] [номер ранга].", 0xFF8FA2)
		end
end
function funCMD.osm()
	if thread:status() ~= "dead" then 
		...
		thread = lua_thread.create(function()
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF}, если готовы начать осмотр или {FF8FA2}Page Down{FFFFFF} для отмены.", 0xFF8FA2)
				addOneOffSound(0, 0, 0, 1058)
...
function funCMD.hilka()
	local id = getNearestID()
		...
		thread = lua_thread.create(function()
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Выбранный игрок: {5BF165}"..name.." ["..id.."]", 0xFF8FA2)
				wait(100)
				sampSendChat("Не волнуйтесь, сейчас я Вам помогу!")
				wait(2100)
				sampSendChat("/do В правой руке медицинский саквояж.")
    ...
    sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Ближайший игрок не найден!", 0xFF8FA2)
	end
end
function funCMD.sob()
	sobWin.v = not sobWin.v
...
function funCMD.info()
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Частые команды:", 0xFF8FA2)
...
function funCMD.expel(par)
	...
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Используйте {a8a8a8}/"..cmdBind[22].cmd.." [id игрока] [Причина].", 0xFF8FA2)
	end
end
...
function funCMD.update()
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Производится скачивание новой версии скрипта...", 0xFF8FA2)
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Произошла ошибка при скачивании обновления. Активация резервого источника...", 0xFF8FA2)
		...
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Скачивание завершено, перезагрузка библиотек...", 0xFF8FA2)
			reloadScripts()
			showCursor(false)
		end
	end)
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				print("{FF0000}Ошибка при попытке скачать файл.") 
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Произошла ошибка при скачивании обновления. Похоже, скачиванию что-то мешает.", 0xFF8FA2)
...
function funCMD.updateCheck()
	sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Проверка наличия обновлений...", 0xFF8FA2)
				...
				if doesFileExist(dirml.."/MedicalHelper/files/update.med") then
					...
					urlupd = upd.url
						if upd.version == scr.version then
							sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: Всё отлично, Вы используете самую новую версию скрипта.", 0xFF8FA2)
						else
							sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: {4EEB40}Имеется обновление.{FFFFFF} Напиши {22E9E3}/update{FFFFFF} для получения информации.", 0xFF8FA2)
							wait(5000)
							updWin.v = true
						end
					end
...
function hook.onSendDialogResponse(id, but, list)
	if sampGetDialogCaption() == ">{FFB300}Посты" then
		if but == 1 then
			...
			sampAddChatMessage("{FFFFFF}[{FF8FA2}MedHelper{FFFFFF}]: На карте была выставлена метка места назначения.", 0xFF8FA2)
			addOneOffSound(0, 0, 0, 1058)
		elseif but == 0 then
		end
	end
